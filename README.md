...
s.get() # 이 경우 큐 객체에서 꺼낼 데이터가 없으므로 None 이나 False를 리턴하거나 예외를 발생시킬 것으로 기대되나, 실제 실행해 보면 프로그램이 멈춰버린다. get() 메소드는 빈 스택에서 실행되면 다른 쓰레드가 메소드를 처리해 줄 때까지 무한 대기 상태를 발생시키기 때문이다.

s.get_nowait() # 이렇게 get_nowait()메소드를 사용하면 더이상 스택에 꺼낼 데이터가 없을 때 바로 queue.Empty 예외를 발생시킨다.
